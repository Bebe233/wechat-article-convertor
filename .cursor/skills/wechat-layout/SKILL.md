...
name: wechat-layout
description: >-
  Converts Chinese marketing draft raw.md into paste-ready WeChat HTML using
  project snippets and design tokens. Use when raw.md is confirmed and the user
  needs wechat.html, WeChat公众号排版, or layout from articles slug folder.
disable-model-invocation: true
---

# WeChat Layout

将已确认的 `articles/<slug>/raw.md` 转为可粘贴到公众号后台的 `wechat.html`。

## 前置条件

- 用户已确认 `raw.md`（pipeline 卡点 2 通过）
...
3. 图片：默认输出 `<!-- IMAGE: {desc} | 比例 {ratio} | 备注 {optional} -->`；不在 HTML 中写外链 `<img>`
4. 若用户允许且环境支持 GenerateImage：可写入 `articles/<slug>/assets/`，HTML 仍保留 IMAGE 注释并注明本地文件名
5. 允许外层单个 `<section>` 包裹；文首用 HTML 注释说明主题与复制步骤
